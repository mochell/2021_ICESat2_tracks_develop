...
plot_path   = mconfig['paths']['plot'] + '/'+hemis+'/'+batch_key+'/vids/' + track_name + '/'
...
for gi in zip(ggg.flatten(), xxx.flatten()):
    ...
    for k_prime_max, Z_max in zip(k_list, weight_list):
        ...
        F = M.figure_axis_xy(fig_sizes['one_column_high'][0]*1.2,fig_sizes['one_column_high'][1], view_scale = view_scale, container = True)
        title_str =  '$\lambda=$'+ str(np.round(2 * np.pi/k_prime_max, 1)) + 'm'
        plt.suptitle(title_str, y = 0.92)
        gs = GridSpec(12, 5,  wspace=0.3,  hspace=5.5)#figure=fig,
        F.gs = gs
        # y_offset= 0.5
        ...
        F.ax2.spines['bottom'].set_visible(False)
        ...
        # no prior
        F.ax3 = F.fig.add_subplot(gs[4:7, 0:-1])
        ...
        F.ax4 = F.fig.add_subplot(gs[4:7, -1])
        ...
        # with prior
        F.ax5 = F.fig.add_subplot(gs[7:, 0:-1])
        if brute is True:
        ...
        F.ax5.set_xticks([0, np.pi/2, np.pi, np.pi* 1.5, 2 *np.pi])
        F.ax5.set_xticklabels(['0', '', '$\pi$', '', '$2\pi$'])
        ...
        F.ax51 = F.fig.add_subplot(gs[7:, -1])
        F.ax51.tick_params(left= False, labelleft=False)#, right= True, labelright= True)
        ...
        F.ax5.set_ylim(-np.pi/1.1, np.pi/1.1)
        F.ax51.set_ylim(-np.pi/1.1, np.pi/1.1)
