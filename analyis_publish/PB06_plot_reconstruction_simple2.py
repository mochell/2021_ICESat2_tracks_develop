...
plot_path   = mconfig['paths']['plot'] + '/proposal_SWOT/' + ID_name +'/'  
MT.mkdirs_r(plot_path)
font_for_print()
...
F = M.figure_axis_xy(5.5, 3, container =True, view_scale= 0.8)

#plt.suptitle('ALT03 Decomposition\n'+ io.ID_to_str(ID_name), y = 0.93, x = 0.13, horizontalalignment ='left')
plt.suptitle('Surface elevation decomposition\nof 1D ICESat-2 photon retrievals', y = 0.88, x = 0.13, horizontalalignment ='left')
#Photon height reconstruction | x='+str(Gk.x[i].data)+' \n' + ID_name, y = 0.95)
gs = GridSpec(8,6,  wspace=0,  hspace=0.8)#figure=fig,

#ax0 = F.fig.add_subplot(gs[0:6, :])


ax1 = F.fig.add_subplot(gs[2:6, :])
plt.title(' '+next(fn)+ 'Altimeter surface height observation\n     and GFT model', loc='left', y= 0.92)
...
plt.scatter( T3_sel['dist'] , T3_sel['heights_c_weighted_mean'], s=0.8 , color =col_d[k], linewidth = 1, label = 'observed surface height mean', zorder= 10)
...
plt.scatter( T2_sel['dist'] , T2_sel['heights_c'], s= 1,  marker='o', color='black',   alpha =0.02, edgecolors= 'none' , zorder =8  )
...
plt.scatter( T3_sel['dist'] ,  T3_sel['heights_c_model'] , s=0.8 , c=lead_color, linewidth=0.5, alpha=1,zorder= 12, label = 'GFT model')
plt.legend(loc = 4, ncol =2)



ax2 = F.fig.add_subplot(gs[6:, :])
plt.title(' '+next(fn)+ 'Residual surface (sea-ice) elevation', loc='left', y= 0.83)
...
plt.scatter( height_residual_mean_x , height_residual_mean, s=0.6, color =col.rascade1, label = 'residual surface (sea ice) elevation')
...
# INLEY
ax41 = F.fig.add_subplot(gs[0:2, 4:])
